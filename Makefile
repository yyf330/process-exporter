...
dockertest:
	docker run --rm -it -v `pwd`:/go/src/github.com/yyf330/process-exporter golang:1.12  make -C /go/src/github.com/yyf330/process-exporter test

dockerinteg:
	docker run --rm -it -v `pwd`:/go/src/github.com/yyf330/process-exporter golang:1.12  make -C /go/src/github.com/yyf330/process-exporter build integ
